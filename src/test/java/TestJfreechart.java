...
//首先构造数据
        TimeSeries timeSeries = new TimeSeries("BMI", Second.class);
// 时间曲线数据集合
...
// 构造数据集合
        for (DataBean data : dataBeans) {
            timeSeries.add(new Second(data.getDate()), data.getVari());
...
//增加标题
        chart.setTitle(new TextTitle("XXXBMI指数", new Font(null, Font.PLAIN, 15)));
