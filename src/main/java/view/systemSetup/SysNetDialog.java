...
public abstract class SysNetDialog extends JDialog {
    NetBean bean = null;
    ...
    protected void initDefault(Window owner) {
        ...
        JLabel title = new JLabel("网关设置", new ImageIcon(
        ...
        buttonSave.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    NetBean newNetbean = getNetInfo();
                    boolean contains = SysNetService.getNetList().contains(newNetbean);
                    if (bean != null) {
                        if ((!bean.equals(newNetbean)) && contains) {
                            JOptionPane.showMessageDialog(null, "网关已存在", "提示", JOptionPane.INFORMATION_MESSAGE);
                            return;
                        }
                        SysNetService.updateNetBean(newNetbean, bean);
                        ...
                        SysNetService.addNet(newNetbean);
                    ...
                    JOptionPane.showMessageDialog(null, "请输入合法的信道号", "提示", JOptionPane.ERROR_MESSAGE);
     ...
     * 获取需要提交的信息
     */
    protected abstract NetBean getNetInfo() throws Exception;

    /**
     * 加载信息
     */
    protected abstract void loadNetInfo();

    /**
     * 清空表单
     */
    protected abstract void clearNetInfo();
